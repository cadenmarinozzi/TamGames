...
function demangleSymbol(e) {
	return (
		Module.debugSymbols &&
			Module.debugSymbols[e] &&
			(e = Module.debugSymbols[e]),
...
function demangleError(e) {
	...
	return e.replace(n, function (e) {
		...
		return (
			...
			(t[2] != n ? ' [' + t[2] + ']' : '') +
			' (' +
			(i ? a.substr(a.lastIndexOf('/') + 1) + i[2] : t[7]) +
			')'
		);
	});
}
		...
		for (
			...
			if (
				...
				String.fromCharCode.apply(
					null,
					e.subarray(s + 3, s + 3 + i[d].length)
				) === i[d]
			)
				d++;
			else if (
				d != o.SCANNING_MODULE_VARIABLES ||
				u ||
				61 != e[s] ||
				String.fromCharCode.apply(
					null,
					e.subarray(s + 1, s + 1 + r.length)
				) !== r
			) {
				if (u && 40 == e[s]) {
		...
		(o.onload = function () {
			...
			console.log(
				'Decompressed ' +
					...
					' in ' +
					(r - i) +
					'ms. You can remove this delay if you configure your web server to host files using ' +
				...
				function (e, t, n) {
					...
					function u(e) {
						if (ge) {
							...
							for (
								y(S, e.lens, 0, 288, p, 0, e.work, { bits: 9 }),
									t = 0;
								32 > t;

							)
								e.lens[t++] = 5;
							y(x, e.lens, 0, 32, b, 0, e.work, { bits: 5 }),
								(ge = !1);
						}
						(e.lencode = p),
							(e.lenbits = 9),
					...
					function h(e) {
						...
						var t = e.state;
						return (
							t.window && (t.window = null), (e.state = null), D
						);
					}
					function m(e, t) {
						var n;
							...
							c.next_out &&
								...
								('string' === this.options.to
									? ((i = s.utf8border(c.output, c.next_out)),
									  ...
									  this.onData(u))
									: this.onData(
											r.shrinkBuf(c.output, c.next_out)
									  ));
...
var CompressionState = {
	...
	Set: function (e) {
		if (CompressionState.current == CompressionState.Pending) {
			...
			for (
				var t = 0;
				t < CompressionState.pendingServerRequests.length;
				t++
			)
			...
			function n() {
				...
				function o(o) {
					...
					for (i = 0; i < r.length; ++i)
						n.prototype.requests[r[i].filename].onload();
				...
				var r = e.files;
				for (i = 0; i < r.length; ++i)
					new n(r[i].start, r[i].end, r[i].crunched, r[i].audio).open(
						'GET',
						r[i].filename
					);
